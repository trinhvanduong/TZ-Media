...
class TZ_MediaControllerFolder extends JControllerLegacy
{
		...
		if (count($paths))
		{
			foreach ($paths as $path)
			{
				if ($path !== JFile::makeSafe($path))
				{
					...
					JError::raiseWarning(100, JText::sprintf('COM_TZ_MEDIA_ERROR_UNABLE_TO_DELETE_FOLDER_WARNDIRNAME', substr($dirname, strlen(COM_TZ_MEDIA_BASE))));
					continue;
				}
	...
	public function create()
	{
		...
		$this->setRedirect('index.php?option=com_tz_media&folder=' . $parent . '&tmpl=' . $this->input->get('tmpl', 'index'));
		...
		if (strlen($folder) > 0)
		{
			...
			{
				$this->setMessage(JText::_('COM_TZ_MEDIA_ERROR_UNABLE_TO_CREATE_FOLDER_WARNDIRNAME'));
				return false;
			}

			$path = JPath::clean(COM_TZ_MEDIA_BASE . '/' . $parent . '/' . $folder);
			if (!is_dir($path) && !is_file($path))
			{
				...
				{
					// There are some errors in the plugins
					JError::raiseWarning(100, JText::plural('COM_TZ_MEDIA_ERROR_BEFORE_SAVE', count($errors = $object_file->getErrors()), implode('<br />', $errors)));
					return false;
				}
				...
				$dispatcher->trigger('onContentAfterSave', array('com_tz_media.folder', &$object_file, true));
				$this->setMessage(JText::sprintf('COM_TZ_MEDIA_CREATE_COMPLETE', substr($path, strlen(COM_TZ_MEDIA_BASE))));
			}
