...
//			$folder = array_diff_assoc($folder, explode('/', COM_TZ_MEDIA_ROOT));
//			array_pop($folder);
//			$folder = implode('/', $folder);
//		}
//		elseif (file_exists(JPATH_ROOT . '/' . COM_TZ_MEDIA_ROOT . '/' . $directory))
        if (file_exists(JPATH_ROOT . '/' . COM_TZ_MEDIA_ROOT . '/' . $directory))
		{
			$folder = $directory;
		}
		else
		...
		if ($this->element['disabled'] != true)
		{
			...
			$html[] = '<a class="modal btn" title="' . JText::_('JLIB_FORM_BUTTON_SELECT') . '"' . ' href="'
				...
				: ($link ? $link
					: 'index.php?option=com_tz_media&amp;view=images&amp;tmpl=component&amp;asset=' . $asset . '&amp;author='
